document.addEventListener("DOMContentLoaded", async () => {
    loadHeader("Remove Accounts from Store");

    const current_accounts = await fetchCurrentAccounts();
    displayCurrentAccounts(current_accounts);

    const old_accounts = await fetchOldAccounts();
    displayOldAccounts(old_accounts)
});
...
function displayOldAccounts(accounts) {
    const container = document.getElementById("old-accounts-list");
...
async function fetchOldAccounts() {
    try{
        const response = await fetch("http://127.0.0.1:8000/old-accounts/");
        if (response.ok) {
            const data = await response.json();
            return data.accounts;
        } else {
            console.error("Failed to fetch old accounts:", response.status);
            return [];
        }
    } catch (error){
...
document.getElementById("remove-accounts").addEventListener("click", async () => {
    try {
